...
La [récupération locale des conteneurs orphelins](RECUPERATION_RUNTIME.md) exige l'identifiant exact inspecté, le montage personnel attendu et les verrous du propriétaire. Elle clôture les baux expirés sans rejouer les actions ni effacer les fichiers. Elle est réservée à l'exploitation, sans endpoint administrateur client. La [restauration locale](SAUVEGARDE_RESTAURATION.md) conserve les données privées et suspend les actions dans des cibles neuves. Le [chiffrement avec des clés de récupération distinctes](CHIFFREMENT_SAUVEGARDES.md) est implémenté. La collecte des reçus opérateur, le stockage indépendant et les exercices de reprise restent à mettre en place sur l’hébergement.
