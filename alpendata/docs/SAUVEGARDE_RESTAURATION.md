...
Le dossier contient des données personnelles. Cette première commande produit un ensemble **local non chiffré**, avec un dossier en mode `0700` et des fichiers en `0600`. Elle n’effectue aucun transfert distant. Chiffrement, copie indépendante, rétention et tests de récupération depuis le stockage Infomaniak restent à intégrer avant exploitation. Les empreintes détectent une altération accidentelle ; elles ne prouvent pas l’authenticité face à quelqu’un capable de modifier aussi le manifeste.
