...
Les commandes `alpendata_api.chat_worker` et `alpendata_api.schedule_worker` vérifient désormais la connexion et la révision de la base avant leur première itération. Elles interceptent `SIGTERM` et `SIGINT` : l'itération déjà admise se termine, puis le processus quitte avec le code zéro. Le worker chat ne prend pas le travail suivant. Le planificateur termine son lot courant, qui peut examiner jusqu'à 100 échéances, puis quitte sans attendre les 30 secondes du prochain contrôle.
...
Le générateur `alpendata_api.service_units` prépare trois unités **systemd utilisateur** : API, file de travail et planificateur. Il ne crée pas de compte Linux, ne configure pas sa persistance après déconnexion et n'installe ni n'active les unités. Il refuse un dossier de sortie déjà présent.
...
`Type=exec` confirme que le programme a été exécuté ; l'état systemd « active » ne garantit pas que tous ses contrôles de démarrage ont réussi. Vérifier aussi `/health/ready` pour l'API et l'événement `ready` des workers. Leurs événements de cycle de vie contiennent seulement le nom de service et un état : `ready`, `stopped`, `database_unavailable`, `configuration_failed` ou `failed`. Les exceptions de boucle donnent un code de sortie non nul sans leurs paramètres privés. Les journaux de l'API ont leur propre politique ; `--no-access-log` désactive ses journaux d'accès HTTP. Les cores des trois services sont désactivés et le masque de création des fichiers est `0077`.

Pour une maintenance cohérente :

1. Fermer les nouvelles admissions HTTP au niveau de l'entrée web.
2. Arrêter le planificateur et attendre la fin de son lot courant.
