...
L'entrée HTTPS et le contrôle de compatibilité de la base au démarrage sont préparés ; voir [Entrée HTTPS](ENTREE_HTTPS.md). Les [unités systemd utilisateur et l'arrêt propre des workers](SERVICES_CONTINUS.md) sont préparés. Leur installation persistante, la supervision globale et la validation sur Infomaniak restent à réaliser.
