...
@pytest.fixture
def service_factory(database_url):
    @contextmanager
    def build(settings=None, provider=None):
        settings = settings or Settings(database_url=database_url)
        app = create_app(settings, signin_provider=provider)
