...
**Invitations :** leur acceptation exige une adresse vérifiée par le serveur. Une adresse déclarée dans un jeton Microsoft ne suffit pas ; le mécanisme de vérification de la boîte destinataire reste à implémenter. Les scénarios d’invitation actuels utilisent des adresses synthétiques vérifiées dans leurs fixtures. Un utilisateur nouvellement connecté avec Microsoft peut créer une entreprise, mais ne peut pas encore terminer le parcours d’une invitation réelle.
