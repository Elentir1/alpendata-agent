...
test('chat keeps composition visible, renders results, and exposes personal tools on desktop and mobile', async ({ page }, testInfo) => {
  ...
  let turns: object[] = [], sent = 0, deleted = 0, feedback = '';
  await page.route('**/api/**', async route => {
    ...
    if (path.endsWith('/auth/options')) result = { password: true, microsoft: false, invitation_email: false };
    ...
    else if (path.endsWith('/notifications')) result = { notifications: [], next_before: null, unread: 0 };
    else if (path.endsWith('/events')) { await route.fulfill({ contentType: 'text/event-stream', body: ': connected\n\n' }); return; }
    else if (path.endsWith('/work-inbox')) result = { items: [], unread: 0 };
    else if (path.endsWith('/knowledge')) result = { entries: [] };
    else if (path.endsWith('/files')) result = { files: [], editor_available: false };
    else if (path.endsWith('/infomaniak')) result = { available: false, status: 'disconnected', capabilities: [], allowed_capabilities: [] };
    ...
    else if (path.endsWith('/search-resources')) result = { results: [], next_offset: null };
    else if (path.endsWith('/chat/projects')) result = { projects: [] };
    else if (path.endsWith('/chat/conversations') && method === 'POST') { conversations = [{ id: 'example-chat', title: 'Nouvelle conversation', language: 'fr', created_at: Date.now() / 1000, model: 'zai-glm-5-2', tool_revision: 7 }]; result = conversations[0]; }
    else if (path.endsWith('/chat')) result = { available: true, model: 'zai-glm-5-2', conversations, next_offset: null };
    else if (path.endsWith('/feedback')) { feedback = route.request().postDataJSON().outcome; result = { outcome: feedback }; }
      ...
      turns = [{ ...body, id: 'turn-one', sequence: 1, status: 'completed', response, feedback_available: true, cancel_requested: false, artifacts: [{ id: 'doc-one', filename: 'atelier.docx', media_type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', size: 12000 }] }];
      result = turns[0];
    }
    else if (path.includes('/chat/conversations/') && method === 'DELETE') { deleted++; conversations = []; turns = []; await route.fulfill({ status: 204 }); return; }
  ...
  await page.getByRole('button', { name: 'Utile', exact: true }).click();
  expect(feedback).toBe('useful');
  ...
  await page.getByText('Anciennes notes personnelles', { exact: true }).click();
  ...
  await page.reload();
  await expect(page.getByLabel('Votre message', { exact: true })).toHaveValue('Mon brouillon reste ici');
  expect(sent).toBe(1);
  await page.getByLabel('Actions de la discussion', { exact: true }).click();
  await page.getByRole('button', { name: 'Supprimer la discussion', exact: true }).click();
  await expect(page.getByRole('dialog')).toBeVisible();
  expect(deleted).toBe(0);
  await page.getByRole('button', { name: 'Annuler', exact: true }).click();
  expect(deleted).toBe(0);
  await page.getByRole('button', { name: 'Supprimer la discussion', exact: true }).click();
  await page.getByRole('button', { name: 'Supprimer cette discussion', exact: true }).click();
  await expect(page.getByRole('heading', { name: 'Qu’allons-nous accomplir ?' })).toBeVisible();
  expect(deleted).toBe(1);
  expect(new URL(page.url()).searchParams.has('conversation')).toBe(false);
  await page.reload();
  await expect(page.getByRole('heading', { name: 'Qu’allons-nous accomplir ?' })).toBeVisible();
  await expect(page.getByLabel('Votre message', { exact: true })).toHaveCount(0);
