...
export function CompanyResourceEditor({ path, item, mode, members, language, done, close }: {
  ...
  const [title, setTitle] = useState(item?.title || ''), [kind, setKind] = useState(item?.kind || 'note');
    ...
    try {
      ...
      if (accessOnly && item) {
        ...
        if (!pending.current) {
          ...
          pending.current = { title: title.trim(), kind, text: kind === 'note' ? text.trim() : '', document,
  ...
  return <form className="resource-editor" aria-label={accessOnly ? t.access : item ? t.edit : t.add} onSubmit={event => { event.preventDefault(); void save(); }}>
    ...
    <fieldset disabled={busy || recovery}>
      {!accessOnly && <>
        <label htmlFor={id + 'title'}>{t.name}</label><input id={id + 'title'} value={title} onChange={event => setTitle(event.target.value)} required maxLength={160} />
        <label htmlFor={id + 'kind'}>{t.kind}</label><select id={id + 'kind'} value={kind} onChange={event => setKind(event.target.value as 'note' | 'document')}><option value="note">{t.note}</option><option value="document">{t.document}</option></select>
        {kind === 'note' ? <><label htmlFor={id + 'text'}>{t.text}</label><textarea id={id + 'text'} value={text} onChange={event => setText(event.target.value)} required maxLength={20000} rows={6} /></> : <><p>{t.formats}</p><label htmlFor={id + 'file'}>{t.file}</label><input id={id + 'file'} type="file" accept=".pdf,.docx,.xlsx,.pptx" required onChange={event => setFile(event.target.files?.[0] || null)} /></>}
      </>}
      <label htmlFor={id + 'audience'}>{t.audience}</label><select id={id + 'audience'} value={audience} onChange={event => setAudience(event.target.value as 'team' | 'selected')}><option value="selected">{t.selected}</option><option value="team">{t.team}</option></select>
      {audience === 'selected' && <div className="resource-grants">{members.filter(m => m.active && m.role !== 'admin').map(m => <label key={m.user_id}><input type="checkbox" checked={grants.includes(m.user_id)} onChange={event => setGrants(current => event.target.checked ? [...current, m.user_id] : current.filter(uid => uid !== m.user_id))} />{m.display_name}</label>)}{!grants.length && <p>{t.onlyAdmins}</p>}</div>}
    ...
    <div className="resource-actions">
      ...
      <button className="secondary" type="button" disabled={busy} onClick={close}>{recovery ? t.reload : t.close}</button>
    </div>
  </form>;
}
