...
import { KDriveSave } from './KDriveSave';
...
export function Documents({ items, organizationId, language }: { items: DocumentReceipt[]; organizationId: string; language: Language }) {
  ...
  const [kdrive, setKdrive] = useState<DocumentReceipt | null>(null);
  ...
  return <section className="chat-documents" aria-label={fr ? 'Documents créés' : 'Created documents'}>
    ...
    <div className="save-shortcuts">{items.map(item => <div className="document-actions" key={item.id}><button type="button" className="secondary" disabled={!!sharing} onClick={() => setSaving(item)}>{fr ? 'Enregistrer' : 'Save'} {item.filename} {fr ? 'dans Microsoft 365' : 'to Microsoft 365'}</button><button type="button" className="secondary" disabled={!!saving || !!sharing} onClick={() => setKdrive(item)}>{fr ? 'Enregistrer dans kDrive' : 'Save to kDrive'}</button><button type="button" className="secondary" disabled={!!saving || !!sharing} onClick={() => setSharing(item)}>{fr ? 'Partager' : 'Share'} {item.filename} {fr ? 'dans l’entreprise' : 'with the company'}</button></div>)}</div>
    {saving && <SharePointSave key={saving.id} item={saving} organizationId={organizationId} language={language} onClose={() => setSaving(null)} />}
    {kdrive && <KDriveSave item={kdrive} organizationId={organizationId} language={language} close={() => setKdrive(null)} />}
