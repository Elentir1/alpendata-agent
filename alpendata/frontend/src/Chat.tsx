...
import type { CalendarReceipt } from './CalendarReview';
...
import { linkedConversation, readChatValue, rememberConversation, writeChatValue, useConversationDraft, readPendingMessage, savePendingMessage } from './conversationStorage';
import { RunActivity } from './RunActivity';
import { PublishConversation } from './ProjectResources';
import { WorkOptions, defaultWorkSettings } from './WorkOptions';
import type { WorkConfiguration, WorkSettings } from './WorkOptions';
...
type Conversation = { match?: { text: string; filename?: string; sequence?: number }; work_settings?: WorkSettings; integration_provider?: string; id: string; title: string; language: Language; created_at: number; project_id?: string | null; archived?: boolean; model?: string; tool_revision?: number; pinned?: boolean; parent_id?: string; branch_sequence?: number };
type Source = { kind: string; label: string; url: string | null };
type Turn = { feedback_available?: boolean; feedback?: string | null; specialist_models?: string[]; calendar_actions?: CalendarReceipt[]; emails?: EmailReceipt[]; artifacts?: DocumentReceipt[]; sources?: Source[]; id: string; request_id: string; sequence: number; message: string; response: string | null; partial_response?: string | null; status: string; error_code: string | null; cancel_requested: boolean };
type Detail = Conversation & { proposals?: Proposal[]; trials?: Trial[]; turns: Turn[]; next_after: number | null };
type Listing = { service_features?: { research?: boolean; dictation?: string; vision?: string }; connections?: { provider: string; capabilities: string[] }[]; available: boolean; model?: string; conversations: Conversation[]; next_offset: number | null };
...
export function Chat({ organizationId, userId = '', licensed, language, t, initialConversationId = '', onManage, navigation, companyName }: { userId?: string; navigation?: ReactNode; companyName?: string; onManage?: () => void; initialConversationId?: string; organizationId: string; licensed: boolean; language: Language; t: Text }) {
  const c = words[language], base = `/api/organizations/${organizationId}/chat`;
  const scope = `${userId}.${organizationId}`;
  const [listing, setListing] = useState<Listing | null>(null), [selected, setSelected] = useState(() => initialConversationId || linkedConversation(organizationId) || (userId ? readChatValue(`${scope}.selected`) : ''));
  const [detail, setDetail] = useState<Detail | null>(null);
  const latestDetail = useRef<Detail | null>(null);
  const deletedIds = useRef(new Set<string>());
  const [draft, setDraft] = useConversationDraft(scope, selected);
  const [busy, setBusy] = useState(false), [error, setError] = useState<unknown>(null), [revision, setRevision] = useState(0);
  const [uncertain, setUncertain] = useState(false);
  const [newConfiguration, setNewConfiguration] = useState<WorkConfiguration>({ work_settings: defaultWorkSettings });
  const [publishing, setPublishing] = useState(false), [deleting, setDeleting] = useState(false);
  ...
  useEffect(() => { latestDetail.current = detail; }, [detail]);
  useEffect(() => {
    pending.current = readPendingMessage(scope, selected);
    setUncertain(!!pending.current);
  }, [scope, selected]);
  useEffect(() => { if (nextDraft.current) setDraft(nextDraft.current); nextDraft.current = ''; followBottom.current = true; setAwayFromBottom(false); if (userId) rememberConversation(scope, organizationId, selected); }, [selected]);
  useEffect(() => { if (followBottom.current && transcript.current) transcript.current.scrollTop = transcript.current.scrollHeight; }, [detail]);
  useEffect(() => {
    let active = true;
    const timer = setTimeout(() => api<Listing>(listUrl).then(value => { if (active) { setListing(value); setSelected(current => (current && (!project && !archived || !!search)) || value.conversations.some(item => item.id === current) ? current : !project && !search && !archived && initialConversationId && !deletedIds.current.has(initialConversationId) ? initialConversationId : value.conversations[0]?.id || ''); } }).catch(cause => { if (active) setError(cause); }), search ? 250 : 0);
    return () => { active = false; clearTimeout(timer); };
  }, [listUrl, initialConversationId, listRevision]);
  useEffect(() => {
    if (!selected) { setDetail(null); return; }
    let active = true, timer: ReturnType<typeof setTimeout>;
    let previous = latestDetail.current?.id === selected ? latestDetail.current : null;
    if (!previous) setDetail(null);
    async function refresh() {
      try {
        ...
        if (pending.current && previous.turns.some(turn => turn.request_id === pending.current?.request_id)) {
          savePendingMessage(scope, selected, null);
        ...
        timer = setTimeout(refresh, running && typeof EventSource === 'undefined' ? 1500 : 10000);
      } catch (cause) {
        if (!active) return;
        setError(cause);
  ...
  async function send() {
    ...
    savePendingMessage(scope, selected, request);
    try {
      ...
      else { savePendingMessage(scope, selected, null); pending.current = null; setUncertain(false); }
      throw cause;
    }
    const list = await api<Listing>(listUrl);
    if (alive.current) setListing(list);
  }
  async function createConversation(prompt = '', contextProject = project) {
    const created = await api<Conversation>(base + '/conversations', { language, ...newConfiguration, ...(contextProject && contextProject !== 'unfiled' ? { project_id: contextProject } : {}) });
    if (!alive.current) return;
    nextDraft.current = prompt;
    setListing(value => value && { ...value, conversations: [created, ...value.conversations] });
    setSelected(created.id); setProject(contextProject); setSearch(''); setArchived(false); setPanel(null);
  ...
  function openConversation(id: string) {
    setSelected(id); setError(null); setPanel(null);
    setProject(''); setSearch(''); setArchived(false); setListRevision(value => value + 1);
  }
  async function branch(sequence: number, prompt = '', configuration?: WorkConfiguration) {
    const key = `${scope}.branch.${selected}`, parameters = { through_sequence: sequence, ...configuration };
    let request: { request_id: string; parameters: typeof parameters } | null = null;
    try { const previous = JSON.parse(readChatValue(key)); if (previous?.request_id && JSON.stringify(previous.parameters) === JSON.stringify(parameters)) request = previous; } catch { /* No pending branch. */ }
    request ||= { request_id: crypto.randomUUID(), parameters }; writeChatValue(key, JSON.stringify(request));
    const created = await api<Conversation>(`${base}/conversations/${selected}/branches`, { request_id: request.request_id, ...parameters });
    writeChatValue(key, '');
    if (!alive.current) return;
    nextDraft.current = prompt;
  ...
  const messages: Record<string, string> = { project_context_revoked: fr ? 'Votre accès au contexte de ce projet a été retiré. Créez une nouvelle discussion personnelle ou demandez au propriétaire de rétablir votre accès.' : 'Access to this project context was revoked. Start a new personal conversation or ask the owner to restore access.', routine_email_not_accepted: language === 'fr' ? 'L’envoi prévu n’a pas été confirmé comme accepté. Consultez son reçu avant toute autre action.' : 'The expected send was not confirmed as accepted. Review its receipt before taking further action.', routine_sources_missing: language === 'fr' ? 'Les sources nécessaires n’ont pas été consultées. Vérifiez vos connexions.' : 'The required sources were not consulted. Check your connections.', routine_occurrence_expired: language === 'fr' ? 'Le créneau de cette occurrence est dépassé.' : 'This occurrence is past its execution window.', chat_not_configured: c.unavailable, agent_already_running: c.busy, chat_model_changed: c.changed, agent_recovery_required: c.recovery, onboarding_required: language === 'fr' ? 'Complétez votre profil dans « Mon espace » pour commencer.' : 'Complete your profile in “My workspace” to get started.' };
  ...
  return <section className={`chat-page ${sidebarOpen ? '' : 'history-collapsed'} ${panel || filePanel ? 'workbench-open' : ''}`}>
    {deleting && detail && <DeleteConversation path={`${base}/conversations/${detail.id}`} title={detail.title} language={language} close={() => setDeleting(false)} removed={() => { deletedIds.current.add(detail.id); setDraft(''); savePendingMessage(scope, detail.id, null); pending.current = null; setUncertain(false); setDeleting(false); setSelected(''); setDetail(null); setPanel(null); setFilePanel(false); rememberConversation(scope, organizationId, ''); setListRevision(value => value + 1); }} />}
    <aside id="conversation-sidebar" className="conversation-list" aria-label={c.conversations} inert={!sidebarOpen}>
      ...
      <WorkInbox organizationId={organizationId} language={language} onOpen={openConversation} onManage={onManage} /><div className="history-scroll">
        <ProjectPanel choose={prompt => void action(() => createConversation(prompt))} userId={userId} base={base} language={language} projects={projects} onProjects={setProjects} selected={project} onSelect={value => { setProject(value); }} disabled={busy || uncertain || !licensed} onError={setError} />
        ...
        {listing?.conversations.map((item, index, all) => <div key={item.id}>
          ...
          <button className={`conversation-link ${selected === item.id ? 'selected' : ''}`} aria-current={selected === item.id ? 'page' : undefined} disabled={busy || uncertain} onClick={() => { setSelected(item.id); setError(null); if (window.matchMedia('(max-width: 760px)').matches) setSidebarOpen(false); }}><MessageSquare size={15} /><span>{item.pinned && <b aria-label={fr ? 'Favori' : 'Favorite'}>★ </b>}{item.title}{item.match && <small className="search-excerpt">{item.match.filename && <b>{item.match.filename} · </b>}{item.match.text}</small>}</span></button>
        </div>)}
        ...
        {!!search.trim() && <SearchResources base={base} query={search} project={project} language={language} disabled={busy || uncertain || !licensed} choose={(id, prompt) => void action(() => createConversation(prompt, id))} />}
      </div>
      <div className="history-bottom"><button className="workspace-shortcut" onClick={() => { setFilePanel(false); setPanel(panel ? null : 'actions'); }}><Sparkles size={17} /><span>{fr ? 'Explorer les capacités' : 'Explore capabilities'}</span></button>
    ...
    <div className="chat-conversation">
      <header className="conversation-topbar"><button className="icon-button" aria-label={fr ? 'Afficher les discussions' : 'Show conversations'} aria-expanded={sidebarOpen} aria-controls="conversation-sidebar" onClick={() => setSidebarOpen(value => !value)}><PanelLeftOpen size={19} /></button>
        ...
        <div className="conversation-header-actions">{(detail?.tool_revision || 0) >= 7 && <button onClick={() => { setFilePanel(value => !value); setPanel(null); }}>{fr ? 'Fichiers' : 'Files'}</button>}{detail && <details className="conversation-more"><summary aria-label={fr ? 'Actions de la discussion' : 'Conversation actions'}>···</summary><div>
          <button disabled={busy || !licensed} onClick={() => void action(async () => {
            const updated = await api<Conversation>(`${base}/conversations/${detail.id}`, { title: detail.title, project_id: detail.project_id || null, archived: !!detail.archived, pinned: !detail.pinned }, 'PUT');
            setDetail(current => current && { ...current, ...updated }); setListRevision(value => value + 1);
          })}>{detail.pinned ? fr ? 'Retirer des favoris' : 'Remove favorite' : fr ? 'Ajouter aux favoris' : 'Add favorite'}</button>
          {detail.project_id && <button disabled={!licensed || busy || !!running} onClick={() => setPublishing(true)}>{fr ? 'Publier une copie dans le projet' : 'Publish a copy to project'}</button>}<a href={`${base}/conversations/${detail.id}/export`} download>{fr ? 'Exporter la discussion' : 'Export conversation'}</a>
          <button disabled={busy || !!running || !licensed} onClick={() => void action(() => branch(detail.turns.at(-1)?.sequence || 0))}>{fr ? 'Continuer dans une variante' : 'Continue in a branch'}</button>
          {detail.parent_id && <button onClick={() => openConversation(detail.parent_id!)}>{fr ? 'Ouvrir la discussion d’origine' : 'Open original conversation'}</button>}
          <button disabled={busy || uncertain} onClick={() => setDeleting(true)}>{fr ? 'Supprimer la discussion' : 'Delete conversation'}</button>
        </div></details>}<button className={panel === 'documents' ? 'selected' : ''} onClick={() => { setFilePanel(false); setPanel(panel === 'documents' ? null : 'documents'); }} aria-label={fr ? 'Livrables de la discussion' : 'Conversation deliverables'}><FileText size={17} /><span>{fr ? 'Livrables' : 'Deliverables'}</span>{documents.length > 0 && <b>{documents.length}</b>}</button><button className={panel === 'actions' ? 'selected' : ''} onClick={() => { setFilePanel(false); setPanel(panel ? null : 'actions'); }} aria-label={fr ? 'Ouvrir les capacités de l’agent' : 'Open agent capabilities'}><Sparkles size={17} /><span>{fr ? 'Capacités' : 'Capabilities'}</span></button></div>
      </header>
      <div className="conversation-notices"><Notice>{failureText}</Notice>{listing && !listing.available && <p className="configuration-note">{c.unavailable}</p>}{!licensed && <Notice>{t.license}</Notice>}{modelChanged && <p className="configuration-note">{fr ? 'Un nouveau modèle est disponible. Continuez avec votre historique dans une variante.' : 'A new model is available. Continue with your history in a branch.'}<button disabled={busy || !!running || !licensed} onClick={() => void action(() => branch(detail?.turns.at(-1)?.sequence || 0, draft))}>{fr ? 'Continuer' : 'Continue'}</button></p>}</div>
      <div className="transcript-scroll" ref={transcript} onScroll={event => { const el = event.currentTarget; followBottom.current = el.scrollHeight - el.scrollTop - el.clientHeight < 100; setAwayFromBottom(!followBottom.current); }}>
        <div className="transcript-width">
          <RunActivity base={base} conversationId={selected} language={language} onUpdate={() => setRevision(value => value + 1)} onRevoked={() => { setDetail(null); setListing(null); setError(new ApiError(403, 'agent_access_revoked')); }} />
          {detail?.parent_id && <p className="branch-note">{fr ? 'Variante : les échanges précédents sont conservés comme contexte. Aucune action passée n’a été relancée.' : 'Branch: previous exchanges are retained as context. No past action was repeated.'}</p>}
          {selected && !detail ? <p role="status">{t.loading}</p> : !detail?.turns.length ? <div className="chat-empty"><div className="agent-mark"><Sparkles size={27} /></div><span className="eyebrow">{fr ? 'DU MESSAGE AU RÉSULTAT' : 'FROM MESSAGE TO RESULT'}</span><h2>{fr ? 'Qu’allons-nous accomplir ?' : 'What shall we accomplish?'}</h2><p>{fr ? 'Un document à produire, une décision à préparer, une méthode à transmettre. Votre agent s’en occupe avec vous.' : 'A document to create, a decision to prepare, a workflow to teach. Your agent works on it with you.'}</p><ActionCards language={language} disabled={!canPrepare} choose={prepare} compact /></div> : <div className="chat-messages" aria-label={c.conversations}>
            {detail.turns.map(turn => <div className="chat-turn" key={turn.id}>
              <article className="chat-message from-user"><strong className="sr-only">{c.you}</strong><p>{turn.message}</p><div className="message-actions"><button disabled={busy || !!running || !licensed} onClick={() => void action(() => branch(turn.sequence - 1, turn.message))}>{fr ? 'Modifier dans une variante' : 'Edit in a branch'}</button><button disabled={busy || uncertain} onClick={() => prepare('> ' + turn.message.replaceAll('\n', '\n> ') + '\n\n')}>{fr ? 'Citer' : 'Quote'}</button></div></article>
              {turn.response !== null && <article className="chat-message from-assistant"><div className="assistant-signature"><Sparkles size={16} /><strong>{c.assistant}</strong></div><ChatResponse text={turn.response} language={language} />{turn.feedback_available && <WorkFeedback key={turn.id} path={`${base}/conversations/${selected}/turns/${turn.id}/feedback`} language={language} initial={turn.feedback} />}{!!turn.specialist_models?.length && <p className="subtle">{fr ? 'Modèles spécialisés consultés' : 'Specialist models consulted'} : Mistral · {[...new Set(turn.specialist_models)].join(', ')}</p>}<div className="message-actions"><button disabled={busy || !!running || !licensed} onClick={() => void action(() => branch(turn.sequence - 1, turn.message))}>{fr ? 'Nouvelle réponse dans une variante' : 'New answer in a branch'}</button><button disabled={busy || uncertain} onClick={() => prepare('> ' + (window.getSelection()?.toString() || turn.response || '').slice(0, 16000).replaceAll('\n', '\n> ') + '\n\n')}>{fr ? 'Reprendre ce passage' : 'Continue from this passage'}</button></div>{!!turn.sources?.length && <details className="chat-sources"><summary>{fr ? 'Sources consultées' : 'Sources consulted'} · {turn.sources.length}</summary><ul>{turn.sources.map((source, index) => <li key={index}>{source.url && source.url.startsWith('https://') ? <a href={source.url} target="_blank" rel="noreferrer">{source.label || source.kind}</a> : <span>{source.label || source.kind}</span>}</li>)}</ul></details>}</article>}
              {turn.response === null && turn.partial_response && <article className="chat-message from-assistant"><div className="assistant-signature"><Sparkles size={16} /><strong>AlpenData</strong><small>{fr ? 'En cours' : 'In progress'}</small></div><ChatResponse text={turn.partial_response} language={language} /></article>}
              ...
              {(turn.calendar_actions || []).map(item => <CalendarReview key={item.id} item={item} organizationId={organizationId} language={language} licensed={licensed} />)}
      ...
      <div className="composer-dock">
        ...
        {selected ? <form className="chat-composer" onSubmit={event => { event.preventDefault(); followBottom.current = true; void action(send); }}>
          ...
          <div className="chat-send-actions">{listing?.service_features?.dictation && <Dictation key={selected} path={`${base}/conversations/${selected}`} language={language} model={listing.service_features.dictation} disabled={busy || uncertain || !licensed || modelChanged} insert={text => setDraft(value => (value ? value + '\n\n' : '') + text)} />}<button type="button" className="composer-tools" onClick={() => { setFilePanel(false); setPanel(panel ? null : 'actions'); }}><Plus size={18} /><span>{fr ? 'Outils et méthodes' : 'Tools and workflows'}</span></button><WorkOptions researchAvailable={!!listing?.service_features?.research} language={language} current={detail ? { work_settings: detail.work_settings || defaultWorkSettings, integration_provider: detail.integration_provider } : newConfiguration} connections={listing?.connections || []} disabled={busy || uncertain || !!running || !licensed} existing={!!detail} apply={async value => { setNewConfiguration(value); if (detail) await branch(detail.turns.at(-1)?.sequence || 0, draft, value); }} /><span className="composer-key-hint">{fr ? 'Maj + Entrée pour une nouvelle ligne' : 'Shift + Enter for a new line'}</span>
    ...
    {publishing && detail?.project_id && <PublishConversation base={base} projectId={detail.project_id} title={detail.title} transcript={detail.turns.map(turn => `## ${fr ? 'Vous' : 'You'}\n\n${turn.message}\n\n## AlpenData\n\n${turn.response || ''}`).join('\n\n')} language={language} close={() => setPublishing(false)} />}
    {filePanel && selected && <WorkspaceFiles key={selected} conversationId={selected} organizationId={organizationId} language={language} licensed={licensed} artifacts={documents} close={() => setFilePanel(false)} choose={prepare} />}
    {!filePanel && panel && <AgentWorkbench tab={panel} setTab={setPanel} close={() => setPanel(null)} organizationId={organizationId} language={language} licensed={licensed} documents={documents} choose={prepare} disabled={!canPrepare} onOpen={openConversation} onManage={onManage} />}
