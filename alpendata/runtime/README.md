...
Ce runtime exécute le véritable `AIAgent` du fork dans un conteneur Linux sans réseau externe. Il fournit la brique d’exécution du futur chat ; les routes de conversation et la passerelle Mistral/OpenRouter ne sont pas encore reliées à cette brique.
...
- Un nom de conteneur stable par propriétaire empêche un second démarrage après une panne du superviseur. Si un ancien conteneur subsiste, l’exécution demande une récupération explicite ; elle ne supprime pas silencieusement un travail dont l’état est inconnu. La réconciliation automatique avec les futurs états de conversation reste à relier.
...
Le message initial contient l’identifiant serveur de conversation, le modèle configuré, le message utilisateur, le contexte système de la conversation, l’historique conservé côté serveur et les capacités métier disponibles. Le processus démarre un endpoint de modèle local à adresse stable, applique l’identité AlpenData et réutilise la mémoire privée Hermes. Les outils de fichiers, terminal et mémoire restent ceux d’Hermes ; les outils Microsoft appellent la passerelle.
...
Les frames sont plafonnées à 8 Mio et les demandes par tour à 80. L’écriture sur les pipes ne bloque pas le contrôle du délai. Le runtime ne conserve ni stderr ni corps d’échange dans les journaux d’exploitation. Le résultat contient la réponse et les messages Hermes ; le service de conversation devra en contrôler la propriété, l’état et la conservation.
...
Un test de qualité avec Mistral/OpenRouter, les contrôles métier pendant une conversation et l’intégration du chat web restent nécessaires avant le pilote. Le moteur n’exécute pas encore les routines du client.
